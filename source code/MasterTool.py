...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Master tool, allows you to select the different tools I created')

    # Non optional argument, this is the specific tool that we wish to call
    parser.add_argument(
        'tool', choices=functions.FUNCTION_MAP.keys(), help=toolHelpString)
    # Directory the tool will operate on, defaults to current directory
    parser.add_argument('-d', '--directory', nargs='?', type=str, default='./',
                        help='Choose a directory for the tool to operate on, defaults to your current directory')
